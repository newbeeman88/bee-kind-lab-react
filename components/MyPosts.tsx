      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
        <div>
          <div className="flex items-center space-x-2 sm:space-x-3 mb-2">
            <SleepyBeeIcon size={40} className="sm:w-10 sm:h-10" />
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground" style={{ fontFamily: 'var(--font-family-heading)' }}>
              My Posts 📝
            </h1>
          </div>
          <p className="text-sm sm:text-base lg:text-lg text-muted-foreground" style={{ fontFamily: 'var(--font-family-primary)' }}>
            Your beekeeping journey documented - {posts.length} {posts.length === 1 ? 'post' : 'posts'}
          </p>
        </div>
        <Button 
          onClick={onCreatePost} 
          className="bg-gradient-to-r from-primary to-secondary text-white hover:from-primary/90 hover:to-secondary/90 px-4 sm:px-6 py-2 sm:py-3 shadow-lg hover:shadow-xl transition-all duration-300 text-sm sm:text-base w-full sm:w-auto"
        >
